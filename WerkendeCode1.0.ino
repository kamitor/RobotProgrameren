...
//TOGGLE
      if(rx_byte == '0'){
      Serial.println("NEW IMPUT RECIEVED");      
        if(GaAan == 1){
         GaAan = 0;
         Serial.println("automatic is set off");
        } else {
         GaAan = 1;
         Serial.println("automatic is set on");
        }
      }
      } else {// number bytes
...
if (GaAan == 1) {
 Serial.println("Enganging automatic search and destroy mode");
 Automatic();
} else {
  stopp();
}
